...
## 2. Start from the reference skeleton
...
- Install `polyglot-accelerator-conformance`.
...
- Publish your driver as `polyglot-accelerator-driver-<vendor>` to the appropriate package registry (PyPI for Python, etc.).
- Link to the conformance badge and the live conformance report.

## 5. Join the working group

- Subscribe to the WG mailing list / Slack channel (linked from the schema repo).
- Propose your vendor's contributions to the next dated schema version as RFCs.
...
??? question "What if my hardware genuinely doesn't have one of the REQUIRED events (e.g. no NVLink-equivalent)?"
    Don't emit it. The conformance suite checks for the *shape* of every event you *do* emit; it doesn't require you to manufacture events for hardware features you don't have. The identity manifest's `topology.intra_node_fabric` field declares what you do have.
